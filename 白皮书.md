...
当一个`Unicast`响应被用户确认，相关服务费用将从托管账户中释放到匹配的服务提供者账户 ——其中扣除少量（由'ServiceFeeTaxRate'定义的）税金并添加到*系统准备金（system reserve）* 中; 同时，相关请求的押金也将退还给服务消费者。
...
对远程的`Multicast`服务的请求以相同的方式处理，只不过可以在源链中生成多个`IBCPacket`。
